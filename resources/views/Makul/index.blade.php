            ...
            <div class="card-body">
              <h5 class="card-title">Data Dosen</h5>
               ...
               <table class="table">
                  ...
                  <tbody>
                      @foreach ($makul as $no => $item)
                    <tr>
                        <td>{{ $makul->firstItem()+$no }}</td>
                        <td><a href="/makul/{{$item->id}}/profile">{{ $item->kode}}</a></td>
                        <td>{{ $item->nama}}</td>
                        <td>{{ $item->semester}}</td>
                        <td>{{ $item->dosen_id}}</td>
                        <td>
                        {{-- <a href="{{ route('makul.edit',$data->id) }}" class="btn btn-light waves-effect waves-light m-1"> <i class="fa fa-edit"></i> </a> --}}
                            <a href="#" data-id="{{ $item->id}}" class="btn btn-light waves-effect waves-light m-1 btn-confirm"> <i class="fa fa-trash-o"></i>
                                <form action="{{ route('makul.destroy', $item->id )}}" id="delete{{ $item->id }}" method="POST">
                                @csrf
                                @method('delete')
                                </form>
                            </a>
